# BMNR — Conviction Journey

> Bitdeer Technologies (ETH Treasury Play)

---

## Entry Thesis (진입 시점 기록)

- **진입 가격**: [미확인 — Shawn에게 확인 필요]
- **배분**: 35%
- **핵심 가설**: MSTR 아날로지. ETH 상승기에 mNAV 배수 확장 + ETH 가격 상승의 이중 레버리지

### Causal Mechanism Map
```
[피아트 자본] → [ETH 매입] → [BMNR Treasury] → [mNAV Premium] → [주가 상승]
                                    ↑
                           시장 ETH narrative 강도 × BMNR 구조 신뢰도
```

**치명적 약점:**
1. ETH 가격 하락 시 mNAV premium 역전 → discount 전환
2. 3인 팀의 불투명한 거버넌스
3. 순수 treasury play로 현금흐름 부재 (MSTR과 차이점)

**대안 경로:**
- 경쟁자 출현 가능성: 낮음 (유사 구조 재현 어려움)
- ETH ETF가 더 나은 대안이 될 가능성: 있음 (단, 레버리지 없음)

### Historical Analogy
```
[THESIS] MSTR 유비: 2022 crypto winter → 생존 → 2024 mNAV 폭등

한계/차이점:
1. MSTR은 기존 소프트웨어 사업의 현금흐름이 있었음 (BMNR은 순수 treasury play)
2. MSTR의 CEO Saylor는 10년+ 트랙레코드 (BMNR은 3인 팀, 불투명)
3. BTC vs ETH의 시장 인식과 규제 리스크 차이
4. 시장 사이클 위치 차이 (MSTR 2020 = 초기, BMNR 2025 = 중기?)

→ 유비 유효성: 50% (부분 적용)
```

### Management Will Tracker
| 항목 | 평가 | 근거 |
|------|------|------|
| Insider Transactions | 🟡 | [확인 필요] |
| Capital Allocation | 🟡 | Treasury 집중, 기타 불투명 |
| Guidance Behavior | 🟡 | 데이터 부족 |
| Skin in the Game | 🟡 | 3인 팀 지분율 불명확 |
| 말 vs 행동 | 🟡 | 데이터 부족 |
| **종합** | 🟡 **Neutral** | 3인 조직, 불투명 |

### Kill Condition
1. **ETH 네트워크 치명적 버그** — 즉시 청산
2. **미국 크립토 전면 금지** — 즉시 청산
3. **BMNR 사기·파산 징후** — 즉시 청산
4. **mNAV 0.7x 이하로 지속** — 재검토
5. **가격 $12 이하** — 손절 검토

### Time Horizon
- **주요 시점**: 2026년 11월 (트럼프 중간선거)
- **모니터링**: ETH 가격, mNAV 배수, 경영진 동향

---

## Conviction Card

| Card | 내용 |
|------|------|
| **C1 🔥 Burn?** | ⭐ \| Risk:Reward = 미정 \| "MSTR 아날로지 + mNAV 이중 레버리지" |
| **C2 🚪 Entry** | 진입 완료 (35%) |
| **C3 🎯 Exit** | [미정 — Shawn 확인 필요] |
| **C4 ☠️ Kill** | ETH fatal bug / US crypto ban / BMNR fraud·bankruptcy / $12 이하 |

---

## Update Log

| 날짜 | 이벤트 | Triage | Delta | Conviction 변화 |
|------|--------|--------|-------|----------------|
| 2026-03-19 | PLEDS 전문가 토론 | — | MSTR analogy 50% valid | ⭐ 유지 |
| 2026-03-20 | 가격 $21.14 (-1.3%) | — | Kill 미충족 | ⭐ 유지 |
| 2026-03-21 | 가격 $20.94 (주간 -10.47%) | **A: 매크로 selloff** | VIX 27, 글로벌 리스크오프 | ⭐ 유지 |

### 2026-03-21 Price Drop Triage
**트리거**: 주간 -10.47% (Triage 발동)

| 가설 | 확률 | 근거 |
|------|------|------|
| **A. 시장 실수** | 60% | (1) VIX 27 급등, 글로벌 리스크오프 환경 (2) 크립토 섹터 전체 하락, BMNR 특수 악재 없음 |
| **B. 펀더멘탈 훼손** | 10% | (1) ETH 네트워크 이슈 없음 (2) 경영진 변화 없음 (3) Kill Condition 미충족 |
| **C. 정보 비대칭** | 30% | (1) 내부자 매도 확인 필요 (2) 공매도 데이터 확인 필요 |

**판정**: A 우세 — 추가 매수 검토 구간이나, C 가설 검증 후 결정

---

## Conviction 에센스

> **"이 기업에 인생을 걸 수 있는가?"**

**현재 답변**: 아직 아니다.

**이유**:
1. MSTR 유비가 50%만 유효
2. 경영진 신뢰도(Management Will)가 🟡 Neutral
3. 순수 treasury play로 현금흐름 부재

**conviction 강화 조건**:
1. ETH 본격 상승기 진입 확인
2. 경영진 투명성 개선 (지분율 공개, 거버넌스 명확화)
3. mNAV premium 지속 확인

**현재 확신도**: ⭐ (관찰 유지, 추가 매수 보류)
